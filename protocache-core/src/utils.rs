...
pub fn compress_into(src: &[u8], out: &mut Vec<u8>) {
    ...
    while pos < src.len() {
        ...
        if pos == src.len() {
            out.push(a);
            emit_run(src, a_start, pos, a, out);
            break;
        }
        let b_start = pos;
        let b = pick_run(src, &mut pos);
        out.push(a | (b << 4));
        emit_run(src, a_start, b_start, a, out);
        emit_run(src, b_start, pos, b, out);
    }
}
...
pub fn decompress_into(src: &[u8], out: &mut Vec<u8>) -> Result<(), ReadError> {
    ...
    match out.len().cmp(&target) {
        std::cmp::Ordering::Less => out.resize(target, 0u8),
        std::cmp::Ordering::Greater => out.truncate(target),
...
fn emit_run(src: &[u8], start: usize, end: usize, mark: u8, out: &mut Vec<u8>) {
    if (mark & 0x8) == 0 {
        let len = end - start;
        if len != 0 {
...
fn unpack(
    ...
    if (mark & 0x8) != 0 {
        ...
        let end = *out_pos + count;
        out[*out_pos..end].fill(if (mark & 0x4) != 0 { 0xff } else { 0x00 });
        *out_pos = end;
        return Ok(());
    }
    let len = (mark & 0x7) as usize;
    let src_remaining = src.len() - *pos;
    let out_remaining = out.len() - *out_pos;
    if len > src_remaining || len > out_remaining {
        return Err(ReadError::new(CorruptionKind::Truncated));
    }
    let end = *pos + len;
